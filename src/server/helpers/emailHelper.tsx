...
import EnrollmentReportEmail from "react-email-starter/emails/enrollment-report";
import EmailInvitation from "react-email-starter/emails/email-invitation";
...
const sendUpdatePreview = async ({
  ...
  if (isStyled) {
    html = render(<EmailInvitation {...data} />);
  } else {
    html = body;
  }
...
const sendCoursePurchasedNotification = async ({
  email,
  courseName,
  name,
  courseId,
  ...
  const html = render(<EnrollmentReportEmail {...data} />);

  const subject = "Thank you for purchasing the course!";
...
export {
  ...
  sendCoursePurchasedNotification,
};
