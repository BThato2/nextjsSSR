...
const Index = () => {
  ...
  const { data: categories, isLoading: catgsLoading } =
    api.courseCategories.getCategories.useQuery();

  const { mutateAsync: createCategoryMutation, isLoading: createCatgLoading } =
    api.courseCategories.createCategory.useMutation();
    ...
    <>
      ...
      <div className="flex min-h-screen w-full flex-col items-start justify-start gap-4 p-8">
        ...
        {catgsLoading ? (
          ...
          >
            {categories?.map((catg) => <li key={catg?.id}>{catg?.title}</li>)}
          </AnimatedSection>
        ) : (
          <AnimatedSection
            delay={0.2}
        ...
        <AnimatedSection className="w-full sm:p-4" delay={0.3}>
            ...
            onSubmit={methods.handleSubmit(async (values) => {
              await createCategoryMutation(values, {
                onSuccess: (catg) => {
                  ...
                  void ctx.courseCategories.getCategories.invalidate();
                },
                onError: () => {
          ...
          >
            <div className="flex w-full items-center gap-4">
              ...
              <button
                type="submit"
                className="flex items-center gap-1 rounded-lg bg-pink-500 px-3 py-2 text-sm font-bold duration-150 hover:bg-pink-600"
              >
                {createCatgLoading ? (
                  <Loader white />
                ) : (
