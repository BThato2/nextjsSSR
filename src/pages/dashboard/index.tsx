...
export default function Dashboard() {
  ...
  const { data: lastLearnedCourses, isLoading: lastLearnedLoading } =
    api.courseEnrollment.getLastLearnedCourses.useQuery();
              ...
              >
                ...
                {lastWeekLearningLoading ? (
                  ...
                  <div className="relative h-60 w-full sm:h-60">
                    ...
                    <LineChart
                      yTickCallback={(value: string | number) => `${value} m`}
                      labels={
                        lastWeekLearning?.map((lwl) =>
                          lwl.date.toLocaleString("en-US", {
                            weekday: "short",
                          }),
                        ) ?? []
                      }
                      // datasetLabel="Minutes learned in previous week"
                      // datasetData={
                      //   lastWeekLearning?.map((lwl) => lwl.minutes) ?? []
                      // }
                      datasets={[
                        {
                      ...
                      tooltipLabelCallback={function (tooltipItem) {
                        return `Minutes Learned: ${
                          (lastWeekLearning ?? [])[tooltipItem.dataIndex]
                            ?.minutes ?? 0
                        }`;
                      }}
                    />
              ...
              {lastWeekLearningLoading ? (
                ...
                <div className="relative h-60 w-full sm:h-60">
                  ...
                  <LineChart
                    ...
                    xTickCallback={(val: string | number) => {
                      return lastWeekLearning
                        ? lastWeekLearning[val as number]?.date?.toLocaleString(
                            "en-US",
                            {
                              day: "numeric",
                              month: "short",
